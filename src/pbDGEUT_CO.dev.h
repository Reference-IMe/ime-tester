...
#ifndef __pbDGEUT_CO_DEV_H__
#define __pbDGEUT_CO_DEV_H__

__attribute__((always_inline)) inline void pbDGEUT_CO_dev (	int mpi_rank_row_in_col, int mpi_rank_col_in_row, int myrows, int mycols, int m,
	...
	if (mpi_rank_col_in_row == l_owner)
	{
		if ( unlikely ( mpi_rank_col_in_row == mpi_rank_row_in_col ) )		// proc holding l-th col AND ON the diagonal
		{
			for (i=0; i < last_row; i++)
			{
				...
				for (j=0; j < i; j++)
				{
					Tlocal[i][j] = Tlocal[i][j]/(1-hi) - lastKr[j]*hh;
				}

				// on diagonal
				Tlocal[i][j] = Tlocal[i][j]/(1-hi);
