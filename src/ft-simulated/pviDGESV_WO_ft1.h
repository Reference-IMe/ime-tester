...
test_output pviDGESV_WO_ft1_sim(int n, double** A, int m, double** bb, double** xx, MPI_Comm comm, int sprocs, int failing_rank, int failing_level)
{
	...
	for (l=n-1; l>0; l--)
	{
		if ((l<=failing_level) && (sprocs>0) && (fault_detected==0)) // time to have a fault!
		{
			...
			*/
			if (rank==failing_rank)
			{
				printf("\n## IMe pviDGESV: bye bye from %d",rank);
			...
			if (rank==spare_rank)
			{
				printf("\n## IMe pviDGESV:  hello from %d, ",rank);
