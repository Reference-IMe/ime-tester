...
#include "../pDGESV-WO-FT.h"

test_result test_IMe_pDGESV_WO_FT (	const char check, const char* tag, const char* variant, int verbosity,
									parallel_env env,
									test_input input,
									int fault_tolerance,
									int faulty_procs,
									...
									int recovery)
{
	...
	else
	{
		...
		if ( strcmp( variant, "PB-WO-BF1-FT") == 0)
		{
			if (fault_tolerance < 1)
			{
				// if fault tolerance is disabled, force disable recovery
				output = pDGESV_WO_FT (A2, bb, xx, input, env, faulty_procs, failing_rank_list, failing_level, 0);
			}
			else
			{
				// if fault tolerance is enabled, pass recovery option as set by user
				output = pDGESV_WO_FT (A2, bb, xx, input, env, faulty_procs, failing_rank_list, failing_level, recovery);
			}
		}
		else
		{
