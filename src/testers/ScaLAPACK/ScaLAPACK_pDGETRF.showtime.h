...
exit_status ScaLAPACK_pDGETRF(int n, double* A_global, double* B_global, int nb,
								...
								int context, int context_global)
{
	...
	result.total_start_time = MPI_Wtime(); // time(NULL);
	SHOWTIME(1)
	...
	if (mpi_rank < cprocs)
	{
		...
		SHOWTIME(4)
		// LU factorization
		result.core_start_time = MPI_Wtime();
		//pdgetrf_( &n, &n, At, &i1, &i1, descAt, ipiv, &info );
		result.core_end_time = MPI_Wtime();
