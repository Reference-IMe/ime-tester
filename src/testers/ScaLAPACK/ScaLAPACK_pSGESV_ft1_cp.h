...
exit_status ScaLAPACK_pSGESV_ft1_cp(int n, float* A_global, int m, float* B_global, int nb,
									...
									int context, int context_global, int context_all, int context_cp)
{
	...
	psgetrf_cp_( &n, &n, At, &i1, &i1, descAt, A_cp, &i1, &i1, descA_cp, ipiv, ipiv_cp, &nipiv, &checkpoint_freq, &failing_level, &context_all, &info );
