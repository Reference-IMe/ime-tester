...
#include "../pviDGESV_WO.nocacheopt.h"


double test_IMe_pviDGESV_nocacheopt(const char* label, int verbosity, int rows, int cols, int nrhs, int rank, int cprocs, int sprocs)
{
	clock_t start, stop;
	double span, maxspan;
	double** A2;
	...
	if (i_calc)
	{
		...
		pviDGESV_WO_nocacheopt(rows, A2, nrhs, bb, xx, comm_calc);
