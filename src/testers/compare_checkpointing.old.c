...
#define MAX_RUNS 10
    ...
    repetitions=1;
    sprocs=1;			// no fault tolerance enabled
...
#define fpinfo(string_label,integer_info) if (main_rank==0 && fp!=NULL) {fprintf(fp,"info,%s,%d\n",string_label,integer_info);}
#define fpdata(track_num) if (main_rank==0 && fp!=NULL) {fprintf(fp,"data,%s,%d,%.0f\n",versionname[track_num],rep+1,versionrun[ track_num][rep]);}
    ...
    for (rep=0; rep<repetitions; rep++)
    {
    	...
    	versionrun[0][rep]=test_ScaLAPACK_pDGETRF(versionname[0], verbose, rows, cols, nb, main_rank, cprocs, sprocs); // SPK LU factorization
 		fpdata(0);

		versionrun[1][rep]=test_ScaLAPACK_pDGETRF_cp_ft1_sim(versionname[1], verbose, rows, cols, nb, main_rank, cprocs, sprocs, failing_rank, -1, checkpoint_skip_interval);	// SPKmod LU factorization single FT and 0 faults
		fpdata(1);

		versionrun[2][rep]=test_ScaLAPACK_pDGETRF_cp_ft1_sim(versionname[2], verbose, rows, cols, nb, main_rank, cprocs, sprocs, failing_rank, failing_level, checkpoint_skip_interval);	// SPKmod LU factorization single FT and 1 fault
		fpdata(2);

    	versionrun[3][rep]=test_ScaLAPACK_pDGEQRF(versionname[3], verbose, rows, cols, nb, main_rank, cprocs, sprocs); // SPK LU factorization
    	fpdata(3);

		versionrun[4][rep]=test_ScaLAPACK_pDGEQRF_cp_ft1_sim(versionname[4], verbose, rows, cols, nb, main_rank, cprocs, sprocs, failing_rank, -1, checkpoint_skip_interval);	// SPKmod QR factorization single FT and 0 faults
		fpdata(4);

		versionrun[5][rep]=test_ScaLAPACK_pDGEQRF_cp_ft1_sim(versionname[5], verbose, rows, cols, nb, main_rank, cprocs, sprocs, failing_rank, failing_level, checkpoint_skip_interval);	// SPKmod QR factorization single FT and 1 fault
		fpdata(5);
