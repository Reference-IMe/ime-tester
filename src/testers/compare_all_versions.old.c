...
#define fpdata(track_num) if (main_rank==0) {fprintf(fp,"data,%s,%d,%.0f\n",versionname[track_num],rep+1,versionrun[ track_num][rep]);}
    ...
    for (rep=0; rep<repetitions; rep++)
    {
    	...
    	versionrun[ 8][rep]=test_ScaLAPACK_pDGESV(versionname[8], verbose, rows, cols, nb, 1, main_rank, cprocs, sprocs);		// SPK solve with 1 rhs
 		fpdata(8);

    	versionrun[ 9][rep]=test_ScaLAPACK_pDGESV(versionname[9], verbose, rows, cols, nb, nRHS, main_rank, cprocs, sprocs);	// SPK solve with 10 rhs
 		fpdata(9);
    	...
    	versionrun[15][rep]=test_ScaLAPACK_pDGETRF(versionname[15], verbose, rows, cols, nb, main_rank, cprocs, sprocs); // SPK LU factorization
 		fpdata(15);

		if (sprocs>0)
    	{
    		versionrun[16][rep]=test_ScaLAPACK_pDGETRF_cp_ft1_sim(versionname[16], verbose, rows, cols, nb, main_rank, cprocs, sprocs, failing_rank, -1, checkpoint_skip_interval);	// SPKmod LU factorization single FT and 0 faults
     		fpdata(16);

     		versionrun[17][rep]=test_ScaLAPACK_pDGETRF_cp_ft1_sim(versionname[17], verbose, rows, cols, nb, main_rank, cprocs, sprocs, failing_rank, failing_level, checkpoint_skip_interval);	// SPKmod LU factorization single FT and 1 fault
     		fpdata(17);
    	...
    	versionrun[20][rep]=test_ScaLAPACK_pDGEQRF(versionname[20], verbose, rows, cols, nb, main_rank, cprocs, sprocs); // SPK LU factorization
    	fpdata(20);

    	if (sprocs>0)
    	{
    		versionrun[21][rep]=test_ScaLAPACK_pDGEQRF_cp_ft1_sim(versionname[21], verbose, rows, cols, nb, main_rank, cprocs, sprocs, failing_rank, -1, checkpoint_skip_interval);	// SPKmod QR factorization single FT and 0 faults
        	fpdata(21);

        	versionrun[22][rep]=test_ScaLAPACK_pDGEQRF_cp_ft1_sim(versionname[22], verbose, rows, cols, nb, main_rank, cprocs, sprocs, failing_rank, failing_level, checkpoint_skip_interval);	// SPKmod QR factorization single FT and 1 fault
        	fpdata(22);
