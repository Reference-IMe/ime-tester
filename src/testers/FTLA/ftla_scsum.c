...
/*
 * @generated s Sun Nov 17 07:03:10 2013
 */
...
static void recover_checksum( char side, float *A, int m, int n, int *descA, ftla_work_t *ftw );
static void recover_abft( float *A, int m, int n, int *descA, ftla_work_t *ftw );
static void hor_checkpointing( float *A, int *descA, int IA, int JA, int iscopy, ftla_work_t *ftw );
static void hor_checkpointing_local (float *A, int *descA, int IA, int JA, int iscopy, ftla_work_t *ftw);
static void recover_local (char side, float *A, int MA, int NA, int *descA, ftla_work_t *ftw);
static void recover_local_fillin (float *A, int MA, int NA, int *descA, ftla_work_t *ftw);
static void create_matrix( int ctxt, float **A, int *descA, int M, int N, int NB, int *np_A, int *nq_A);
 ...
 ***** RECOVERY 
 */
void Cftla_pscsumr ( char zone, float *A, int m, int n, int *descA, ftla_work_t *ftw)
{
    ...
    sprintmatrix(A, descA, "Af_hole", 7);

    // recover checksum
    if( zone == 'C' || zone == 'A' ) {
        recover_checksum ('b', A, m, n, descA, ftw );
        ROOTSAY ("checksum recovered");
        sprintmatrix(A, descA, "Af_csum", 7);
    }
    ...
    if( zone == 'I' || zone == 'A' ) {
        ...
        sprintmatrix(A, descA, "Af_abft", 7);
    }
    ...
    if( zone == 'L' || zone == 'A' ) {
        ...
        sprintmatrix(A, descA, "Af_lcpy", 7);
    }
}
extern void ftla_pscsumr_( char *zone, float *A, int *m, int *n, int *descA, int *ftw ) {
    Cftla_pscsumr( zone[1], A, *m, *n, descA, (ftla_work_t*)ftw );
}

/*
 * Use the duplicate checksum to restore missing checksum blocks
 */
static void recover_checksum (char side, float *A, int MA, int NA, int *descA, ftla_work_t *ftw)
{
    int i, j, k;
    int tt[2], g[2];
    ...
    if (side == 'b' || side == 'r')
    {
        ...
        //Cpsgemr2d (MA-IA+1, nb, A, IA, tc, descA, A, IA, tc+nb, descA, ctxt);
        int i_c, j_c;
        if (iamvic)
        {
            i_c = np_A;
        ...
        for (i=0; i<i_c; i+=nb)
            for (j=0, k=0; j<j_c; j+=nb, k++)
            {
                ...
                Cpsgemr2d (nb, nb, A, tt[0], tt[1], descA, A, g[0], g[1], descA, ctxt);
            }
    }
}
 ...
 */
static void recover_abft (float *A, int MA, int NA, int *descA, ftla_work_t *ftw)
{
    int j;
    int lrindx, lcindx, rsrc, csrc;
    float *Achk_old, *Achk_new;
    ...
    Cpsgemr2d (MA, NAA-NA, A, 1, NA+1, descA, Achk_old, 1, 1, descAchk, ctxt);

    // re-calculate the checksum
    Cftla_pscsum( 'r', A, MA, NA, 1, 1, descA, A, 1, NA+1, descA, ftw );

    // copy the new checksum into buffer
    Cpsgemr2d (MA, NAA-NA, A, 1, NA+1, descA, Achk_new, 1, 1, descAchk, ctxt);

    int N_M = NAA-NA, i1=1;
    float done = 1, mone = -1;
    psmatadd_ ( &MA, &N_M, &done, Achk_old, &i1, &i1, descAchk, &mone, Achk_new, &i1, &i1, descAchk);
    Cpsgemr2d (MA, NAA-NA, Achk_new, 1, 1, descAchk, A, 1, NA+1, descA, ctxt);
    ...
    for (j=0; j<j_c; j+=nb)
    {
        ...
        if (csrc==(ftw->errproc%npcol))
        {
            ...
            if (iamvic)
            {
                ...
                memcpy (A+j*np_A, A+(lcindx-1)*np_A, np_A*nb*sizeof(float));
            }
        }
        else
        {
            ...
            if (myrow == rsrc)
            {
                ...
                if (iamvic)
                {
                    ...
                    MPI_Recv(A+j*np_A, np_A*nb, MPI_FLOAT, csrc, 1, ftw->comms.rowcomm, &status);
                }
                else if (mycol==csrc)
                {
                    //printf ("(%d,%d) sending to (%d,%d) for MPI_COPY when j=%d\n", myrow, mycol, myrow, ftw->errproc%npcol, j);
                    MPI_Send(A+(lcindx-1)*np_A, np_A*nb, MPI_FLOAT, ftw->errproc%npcol, 1, ftw->comms.rowcomm);
                }
            }
        }
    }
    ...
    Cpsgemr2d (MA, NAA-NA, Achk_old, 1, 1, descAchk, A, 1, NA+1, descA, ctxt);
...
static void recover_local_fillin (float *A, int MA, int NA, int *descA, ftla_work_t *ftw)
{
    ...
    // copy the old checksum into buffer
    Cpsgemr2d (MA, nb*2, A, 1, NA+1, descA, Achk_old, 1, 1, descAchk, ctxt);

    // re-calculate the checksum
    hor_checkpointing_local (A, descA, 1, 1, 1, ftw );

    // copy the new checksum into buffer
    Cpsgemr2d (MA, nb*2, A, 1, NA+1, descA, Achk_new, 1, 1, descAchk, ctxt);

    int N_M = nb*2, i1=1;
    float done = 1, mone = -1;
    psmatadd_ ( &MA, &N_M, &done, Achk_old, &i1, &i1, descAchk, &mone, Achk_new, &i1, &i1, descAchk);
    Cpsgemr2d (MA, nb*2, Achk_new, 1, 1, descAchk, A, 1, NA+1, descA, ctxt);
    ...
    // copy the correct checksum back //
    Cpsgemr2d (MA, nb*2, Achk_old, 1, 1, descAchk, A, 1, NA+1, descA, ctxt);
 ...
 */
void Cftla_psqplcpy( float *A, int *descA, int IA, int JA, ftla_work_t *ftw ) {
    ...
    int len = (np_A-lrindx+1)*sizeof(float);
...
extern void ftla_psqplcpy_(float *A, int *descA, int *IA, int *JA, int *ftw ) {
    Cftla_psqplcpy( A, descA, *IA, *JA, (ftla_work_t*)ftw );
}

/**
 * Reset the local copy of the panel in the current Q-wide section
 */
void Cftla_psqplrst( float *A, int *descA, int IA, int JA, ftla_work_t *ftw )
{
#ifndef NO_QLOCALCOPY
    int i;
    float *Aoff;
...
extern void ftla_psqplrst_(float *A, int *descA, int *IA, int *JA, int *ftw ) {
    Cftla_psqplrst( A, descA, *IA, *JA, (ftla_work_t*)ftw );
}
...
//        printf ("(%d,%d) is root (csrc=%d) when JA=%d\n", myrow, mycol, csrc, JA);
    ...
    for (i=0; i<rows; i+=nb)
    {
        ...
        if (in)
        {
            ...
            for (k=0; k<nb; k++)
                memcpy (S+k*nb, Aoff+k*np_A, nb*sizeof(float));
        }
        else
            memset (S, 0, nb*nb*sizeof(float));

        if ((lrindx+i)<np_A)
        {
            MPI_Reduce (S, R, nb*nb, MPI_FLOAT, MPI_SUM, csrc, ftw->comms.rowcomm);

            if (root)
            {
                Aoff = A+(rootcc-1)*np_A+(rootcr-1)+i;  
                for (k=0; k<nb; k++)
                    memcpy (Aoff+k*np_A, R+k*nb, nb*sizeof(float));
            }
        }
    }
 ...
 */
void Cftla_pscsum (char side,
        float *A, int MA, int NA, int IA, int JA, int *descA, 
        float *Acd, int IAcd, int JAcd, int *descAcd,
        ftla_work_t *ftw)
{
    ...
    if (side=='d' || side=='b') {
        ...
        if (nq_A > nqq_A)
            memset (A+np_A*nqq_A, 0, (nq_A-nqq_A)*np_A*sizeof(float));
...
extern void ftla_pscsum_( char *side, float *A, int *MA, int *NA, int *IA, int *JA, int *descA, float *Acd, int *IAcd, int *JAcd, int *descAcd, int *ftw ) {
    Cftla_pscsum( side[1], A, *MA, *NA, *IA, *JA, descA, Acd, *IAcd, *JAcd, descAcd, (ftla_work_t*)ftw );
}
...
static void hor_checkpointing (float *A, int *descA, int IA, int JA, int iscopy, ftla_work_t *ftw)
{
    ...
    if (iscopy && (lcindx<nq_A) && (lrindx+i)<np_A)
    {
        ...
        int len = (np_A-lrindx)*sizeof(float);
        Aoff = A+(lcindx-1)*np_A+(lrindx-1);
        for (i=0; i<nb; i++)
            memcpy (((typeof(A))ftw->pcopy.Pc)+i*np_A, Aoff+i*np_A, len);
    }
 ...
 */
float Cftla_pscsumv( int M, int N, float *A, int *descA, ftla_work_t *ftw) {
    float *Achk_old, *Achk_new;
    int descAchk[9], npp, nqq;
    int nb = descA[NB_];
    int ctxt = descA[CTXT_];
    ...
    // copy the old checksum into buffer
    Cpsgemr2d (M, N-M, A, 1, M+1, descA, Achk_old, 1, 1, descAchk, ctxt);

    // re-calculate the checksum
    Cftla_pscsum ('r', A, M, M, 1, 1, descA, A, 1, M, descA, ftw );

    // copy the new checksum into buffer
    Cpsgemr2d (M, N-M, A, 1, M+1, descA, Achk_new, 1, 1, descAchk, ctxt);

    // cross check
    int N_M = N-M, i1=1;
    float done = 1, mone = -1;
    psmatadd_ ( &M, &N_M, &done, Achk_new, &i1, &i1, descAchk, &mone, Achk_old, &i1, &i1, descAchk);
    float resid = pslange_("F", &M, &N_M, Achk_old, &i1, &i1, descAchk, NULL)/pslange_("F", &M, &N_M, Achk_new, &i1, &i1, descAchk, NULL)/M;
...
extern float ftla_pscsumv_( int *M, int *N, float *A, int *descA, int *ftw ) {
    return Cftla_pscsumv( *M, *N, A, descA, (ftla_work_t*)ftw );
}


static void create_matrix( int ctxt, float **A, int *descA, 
