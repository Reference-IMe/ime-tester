...
/* 
 * @generated s Sun Nov 17 07:17:03 2013
 */

/* psgetrf.f -- translated by f2c (version 20061008).
...
static int c__6 = 6;
static int c__0 = 0;
static float c_b31 = 1.;
static float c_b34 = -1.;
static int c_n1 = -1;

/* Subroutine */ int ftla_psgetrf(int *m, int *n, float *a, int *
        ia, int *ja, int *descA, int *ipiv, int *info, ftla_work_t *ftw)
{
    ...
    extern /* Subroutine */ int psgemm_(char *, char *, int *, int *, 
            int *, float *, float *, int *, int *, 
            int *, float *, int *, int *, int *, 
            float *, float *, int *, int *, int *, 
            ftnlen, ftnlen), blacs_gridinfo__(int *, int *, int *,
                int *, int *), pstrsm_(char *, char *, char *, char *, 
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        pxerbla_(&ictxt, "PSGETRF", &i__1, (ftnlen)7);
        return 0;
    }
    ...
    if ( !Cftla_replay( ftw ) ) {
        Cftla_psqplcpy (&a[1], &descA[1], *ia, *ja, ftw ); /* make a local copy & checkpoint localcopy */
        psgetf2_(m, &jb, &a[1], ia, ja, &descA[1], &ipiv[1], info);
        if( Cftla_procfailed( ftw, *ja ) ) return -1;
    }
    ...
    if (jb + 1 <= realn ) 
    {
        if ( !Cftla_replay( ftw ) ) {
            ...
            i__1 = *n - jb;
            i__2 = jn + 1;
            pslaswp_("Forward", "Rows", &i__1, &a[1], ia, &i__2, &descA[1], ia, &in, 
                &ipiv[1], (ftnlen)7, (ftnlen)4);
                
            /*        Compute block row of U. */
            pstrsm_("Left", "Lower", "No transpose", "Unit", &jb, &i__1, &c_b31, &
        ...
        if (jb + 1 <= *m) 
        {
            ...
            i__5 = in + 1;
            i__6 = jn + 1;
            psgemm_("No transpose", "No transpose", &i__1, &i__2, &jb, &c_b34,
        ...
        if ( !Cftla_repairstep( ftw ) ) {
            ...
            Cftla_pscsumr ('A', &a[1], *m, realn, &descA[1], ftw );
    ...
    for (j=jn+1; i__2<0 ? j>=i__1 : j<=i__1; j+=i__2, jj++) 
    {
        ...
        if ( !Cftla_replay( ftw ) ) {
            i__3 = *m - j + *ja;
            psgetf2_(&i__3, &jb, &a[1], &i__, &j, &descA[1], &ipiv[1], &iinfo);
            if (*info == 0 && iinfo > 0) 
            {
                *info = iinfo + j - *ja;
            }
        ...
        if ( !Cftla_replay( ftw ) ) {
            ...
            i__3 = j - *ja;
            i__4 = i__ + jb - 1;    
            pslaswp_("Forward", "Rowwise", &i__3, &a[1], ia, ja, &descA[1], &i__, 
        ...
        if (j - *ja + jb + 1 <= realn) 
        {
            ...
            if ( !Cftla_replay( ftw ) ) {
                ...
                i__4 = j + jb;
                i__5 = i__ + jb - 1;
                pslaswp_("Forward", "Rowwise", &i__3, &a[1], ia, &i__4, &descA[1],
                    &i__, &i__5, &ipiv[1], (ftnlen)7, (ftnlen)7);

                /*           Compute block row of U. */
                i__3 = *n - j - jb + *ja - dec;
                i__4 = j + jb;
                pstrsm_("Left", "Lower", "No transpose", "Unit", &jb, &i__3, &
            ...
            if (j - *ja + jb + 1 <= *m) 
            {
                ...
                i__7 = i__ + jb;
                i__8 = j + jb;
                psgemm_("No transpose", "No transpose", &i__3, &i__4, &jb, &
       ...
       /*
        * now that psgemm is done, see if we need to Recover 
        */
...
#ifdef VERIFY_CHK
    float resid = Cftla_pscsumv (*m, *n, &a[1], &descA[1], ftw );
 ...
 */
static int psgetrf0( int *m, int *n, float *a, int *ia, int *ja, int *descA, 
                    int *ipiv, int *info, 
                    ftla_work_t *ftw );
static void qpanel_replay( float *A, int MA, int NA, int *descA, ftla_work_t *ftw )
{
    ...
    psgetrf0 (&MM, &NN, A, &jj, &jj, descA, ipiv, &info, ftw );
    free (ipiv);
    ftw->errstep = 0;
    ftw->errproc = -1;
}
...
/* Subroutine */ int psgetrf0(int *m, int *n, float *a, int *
        ia, int *ja, int *descA, int *ipiv, int *info, ftla_work_t *ftw)
{
    /* System generated locals */
    int i__1, i__2, i__3, i__4, i__5, i__6, i__7, i__8;
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        pxerbla_(&ictxt, "PSGETRF0", &i__1, (ftnlen)8);
        return 0;
    }
    ...
    if (jb + 1 <= *n) 
    {
        ...
        i__1 = *n - jb;
        i__2 = jn + 1;
        pslaswp_("Forward", "Rows", &i__1, &a[1], ia, &i__2, &descA[1], ia, &
