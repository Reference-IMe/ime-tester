...
/* 
 * @generated s Sun Nov 17 06:50:31 2013
 */

/* psgeqrf.f -- translated by f2c (version 20061008).
...
extern int ftla_pslarfb(char *side, char *trans, char *direct, char *
...
static void qpanel_replay( float *A, int MA, int NA, int *descA, ftla_work_t *ftw );
...
/* Subroutine */ int ftla_psgeqrf(int *m, int *n, float *a, int *
    ia, int *ja, int *descA, float *tau, float *work, 
    int *lwork, int *info, ftla_work_t *ftw )
{
    ...
    extern /* Subroutine */ int chk1mat_(int *, int *, int *, 
        int *, int *, int *, int *, int *, int *),
         psgeqr2_(int *, int *, float *, int *, int *
        , int *, float *, float *, int *, int *);
    extern int indxg2p_(int *, int *, int *, int *, 
        int *);
    extern /* Subroutine */ int pslarfb_(char *, char *, char *, char *, 
        int *, int *, int *, float *, int *, int 
        *, int *, float *, float *, int *, int *, 
        int *, float *, ftnlen, ftnlen, ftnlen, ftnlen), 
        pslarft_(char *, char *, int *, int *, float *, 
        int *, int *, int *, float *, float *, 
        float *, ftnlen, ftnlen), pxerbla_(int *, char *, 
        int *, ftnlen);
...
/*     Test the input parameters */
    ...
    *info = -602;
    ...
    if (*info != 0) {
    i__1 = -(*info);
    pxerbla_(&ictxt, "FTPSGEQRF", &i__1, (ftnlen)9);
    return 0;
    } else if (lquery) {
    return 0;
    }
...
/*     Quick return if possible */
    ...
    if ( !Cftla_replay( ftw ) ) {
        Cftla_pscsum ('r', &a[1], *m, realn, *ia, *ja, &descA[1], &a[1], 1, N_1, &descA[1], ftw );
...
/*     Compute the QR factorization of the first block A(ia:ia+m-1,ja:jn) */
    ...
    if ( !Cftla_replay( ftw ) ) {
        Cftla_psqplcpy (&a[1], &descA[1], 1, 1, ftw ); /* make a local copy & checkpoint localcopy */
        psgeqr2_(m, &jb, &a[1], ia, ja, &descA[1], &tau[1], &work[1], lwork, &iinfo);
        if( Cftla_procfailed( ftw, *ja ) ) return -1;
    }
    ...
    if (*ja + jb <= *ja + realn - 1) 
    {
        ...
        if ( !Cftla_replay( ftw ) ) {
            pslarft_("Forward", "Columnwise", m, &jb, &a[1], ia, ja, &descA[1], &
        ...
        err = ftla_pslarfb("Left", "Transpose", "Forward", "Columnwise", m, &i__1, &jb, 
...
/*     Loop over the remaining blocks of columns */
    ...
    for (j = jn + 1; i__2 < 0 ? j >= i__1 : j <= i__1; j += i__2, jj++) 
    {
        ...
        {
            if ( !Cftla_replay( ftw ) ) {
                Cftla_psqplcpy (&a[1], &descA[1], j, j, ftw );
        ...
        i__3 = *m - j + *ja;
        if ( !Cftla_replay( ftw ) ) {
            psgeqr2_(&i__3, &jb, &a[1], &i__, &j, &descA[1], &tau[1], &work[1], 
                    lwork, &iinfo);
            sprintsmatrix(i__3, jb, &a[1], i__, j, &descA[1], "QR2f", 4);
            if( Cftla_procfailed( ftw, j ) ) return -1;
        }

#if 0
#ifdef INJECT
        if( -1 == sinject_errors(errors, j, &a[1], &descA[1], ftw ) ) {
            return -1;
        }
#endif
#endif /* 0 */
        ...
        if (((jj+1)%(npcol))==0 || j+i__2>realn) // TODO: check boundary condition (doing extra ckpt at end?)
        {
            ...
            if (mycol+myrow==0)
                ...
                */
            if ( !Cftla_replay( ftw ) ) {
                Cftla_pscsum ('d', &a[1], mn-j+*ja+2*jb, realn, jc, jc, &descA[1], &a[1], *m+1, j, &descA[1], ftw );
                if( Cftla_procfailed( ftw, j ) ) return -1;
            }
        }
        ...
        if (j + jb <= *ja + realn - 1) 
        {
            ...
            if ( !Cftla_replay( ftw ) ) {
                pslarft_("Forward", "Columnwise", &i__3, &jb, &a[1], &i__, &j, &
            ...
            err = ftla_pslarfb("Left", "Transpose", "Forward", "Columnwise", &i__3, 
        ...
        /*
         * now that pslarfb is done, see if we need to Recover 
         */
        if ( Cftla_repairstep( ftw ) ) {
 ...
 */
static int psgeqrf0( int *m, int *n, float *a, int *ia, int *ja, int *descA, 
...
static void qpanel_replay( float *A, int MA, int NA, int *descA, ftla_work_t *ftw )
{
    ...
    Cftla_psqplrst( A, descA, jj, jj, ftw );
    ...
    int lwork = -1;
    float lazywork;
    psgeqrf0 (&MM, &NN, A, &jj, &jj, descA, NULL, &lazywork, &lwork, &info, ftw );
    lwork = (int)lazywork;
    float *work = malloc(lwork*sizeof(*work));
    float *tau = malloc(NA*sizeof(*tau));
    psgeqrf0 (&MM, &NN, A, &jj, &jj, descA, tau, work, &lwork, &info, ftw );
    free (tau);
    free (work);
    ftw->errstep = 0;
    ftw->errproc = -1;
...
/* Subroutine */ static int psgeqrf0(int *m, int *n, float *a, int *
    ia, int *ja, int *descA, float *tau, float *work, 
    int *lwork, int *info, ftla_work_t *ftw)
{
    /* System generated locals */
...
/*     Test the input parameters */
    ...
    *info = -602;
    ...
    if (*info != 0) {
    i__1 = -(*info);
    pxerbla_(&ictxt, "PSGEQRF0", &i__1, (ftnlen)8);
    return 0;
    } else if (lquery) {
    return 0;
    }
...
/*        Apply H' to A(ia:ia+m-1,ja+jb:ja+n-1) from the left */
    ...
    i__1 = *n - jb;
    i__2 = *ja + jb;
    pslarfb_("Left", "Transpose", "Forward", "Columnwise", m, &i__1, &jb, 
...
/*     Loop over the remaining blocks of columns */
    ...
    for (j = jn + 1; i__2 < 0 ? j >= i__1 : j <= i__1; j += i__2, jj++) 
    {
        ...
        if (j + jb <= *ja + *n - 1) 
        {
            ...
            i__4 = *n - j - jb + *ja;
            i__5 = j + jb;
            pslarfb_("Left", "Transpose", "Forward", "Columnwise", &i__3, 
                    &i__4, &jb, &a[1], &i__, &j, &descA[1], &work[1], &a[1], 
