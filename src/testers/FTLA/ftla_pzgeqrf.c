...
/* pzgeqrf.f -- translated by f2c (version 20061008).
...
extern int ftla_pzlarfb(char *side, char *trans, char *direct, char *
...
/* Subroutine */ int ftla_pzgeqrf(int *m, int *n, complexdouble_t *a, int *
    ia, int *ja, int *descA, complexdouble_t *tau, complexdouble_t *work, 
    int *lwork, int *info, ftla_work_t *ftw )
{
    ...
    extern /* Subroutine */ int chk1mat_(int *, int *, int *, 
        int *, int *, int *, int *, int *, int *),
         pzgeqr2_(int *, int *, complexdouble_t *, int *, int *
        , int *, complexdouble_t *, complexdouble_t *, int *, int *);
    extern int indxg2p_(int *, int *, int *, int *, 
        int *);
    extern /* Subroutine */ int pzlarfb_(char *, char *, char *, char *, 
        int *, int *, int *, complexdouble_t *, int *, int 
        *, int *, complexdouble_t *, complexdouble_t *, int *, int *, 
        int *, complexdouble_t *, ftnlen, ftnlen, ftnlen, ftnlen), 
        pzlarft_(char *, char *, int *, int *, complexdouble_t *, 
        int *, int *, int *, complexdouble_t *, complexdouble_t *, 
        complexdouble_t *, ftnlen, ftnlen), pxerbla_(int *, char *, 
        int *, ftnlen);
...
/*  PZGEQRF computes a QR factorization of a real distributed M-by-N */
...
/*     Test the input parameters */
    ...
    *info = -602;
    ...
    if (*info != 0) {
    i__1 = -(*info);
    pxerbla_(&ictxt, "FTPZGEQRF", &i__1, (ftnlen)9);
    return 0;
    } else if (lquery) {
    return 0;
    }
...
/*     Compute the QR factorization of the first block A(ia:ia+m-1,ja:jn) */
    ...
    if ( !Cftla_replay( ftw ) ) {
        Cftla_pzqplcpy (&a[1], &descA[1], 1, 1, ftw ); /* make a local copy & checkpoint localcopy */
        pzgeqr2_(m, &jb, &a[1], ia, ja, &descA[1], &tau[1], &work[1], lwork, &iinfo);
        if( Cftla_procfailed( ftw, *ja ) ) return -1;
    }
    ...
    if (*ja + jb <= *ja + realn - 1) 
    {
        ...
        if ( !Cftla_replay( ftw ) ) {
            pzlarft_("Forward", "Columnwise", m, &jb, &a[1], ia, ja, &descA[1], &
        ...
        err = ftla_pzlarfb("Left", "Transpose", "Forward", "Columnwise", m, &i__1, &jb, 
...
/*     Loop over the remaining blocks of columns */
    ...
    for (j = jn + 1; i__2 < 0 ? j >= i__1 : j <= i__1; j += i__2, jj++) 
    {
        ...
        i__3 = *m - j + *ja;
        if ( !Cftla_replay( ftw ) ) {
            pzgeqr2_(&i__3, &jb, &a[1], &i__, &j, &descA[1], &tau[1], &work[1], 
                    lwork, &iinfo);
            zprintsmatrix(i__3, jb, &a[1], i__, j, &descA[1], "QR2f", 4);
            if( Cftla_procfailed( ftw, j ) ) return -1;
        }

#if 0
#ifdef INJECT
        if( -1 == zinject_errors(errors, j, &a[1], &descA[1], ftw ) ) {
            return -1;
        }
#endif
#endif /* 0 */
        ...
        if (j + jb <= *ja + realn - 1) 
        {
            ...
            if ( !Cftla_replay( ftw ) ) {
                pzlarft_("Forward", "Columnwise", &i__3, &jb, &a[1], &i__, &j, &
            ...
            err = ftla_pzlarfb("Left", "Transpose", "Forward", "Columnwise", &i__3, 
 ...
 */
static int pzgeqrf0( int *m, int *n, complexdouble_t *a, int *ia, int *ja, int *descA, 
                    complexdouble_t *tau, complexdouble_t *work, int *lwork, int *info, 
                    ftla_work_t *ftw );
static void qpanel_replay( complexdouble_t *A, int MA, int NA, int *descA, ftla_work_t *ftw )
{
    ...
    int info;
    int lwork = -1;
    complexdouble_t lazywork;
    pzgeqrf0 (&MM, &NN, A, &jj, &jj, descA, NULL, &lazywork, &lwork, &info, ftw );
    lwork = (int)lazywork;
    complexdouble_t *work = malloc(lwork*sizeof(*work));
    complexdouble_t *tau = malloc(NA*sizeof(*tau));
    pzgeqrf0 (&MM, &NN, A, &jj, &jj, descA, tau, work, &lwork, &info, ftw );
    free (tau);
    free (work);
    ftw->errstep = 0;
    ftw->errproc = -1;
...
/* Subroutine */ static int pzgeqrf0(int *m, int *n, complexdouble_t *a, int *
    ia, int *ja, int *descA, complexdouble_t *tau, complexdouble_t *work, 
    int *lwork, int *info, ftla_work_t *ftw)
{
    /* System generated locals */
...
/*     Test the input parameters */
    ...
    *info = -602;
    ...
    if (*info != 0) {
    i__1 = -(*info);
    pxerbla_(&ictxt, "PZGEQRF0", &i__1, (ftnlen)8);
    return 0;
    } else if (lquery) {
    return 0;
    }
...
/*        Apply H' to A(ia:ia+m-1,ja+jb:ja+n-1) from the left */
    ...
    i__1 = *n - jb;
    i__2 = *ja + jb;
    pzlarfb_("Left", "Transpose", "Forward", "Columnwise", m, &i__1, &jb, 
...
/*     Loop over the remaining blocks of columns */
    ...
    for (j = jn + 1; i__2 < 0 ? j >= i__1 : j <= i__1; j += i__2, jj++) 
    {
        ...
        if (j + jb <= *ja + *n - 1) 
        {
            ...
            i__4 = *n - j - jb + *ja;
            i__5 = j + jb;
            pzlarfb_("Left", "Transpose", "Forward", "Columnwise", &i__3, 
                    &i__4, &jb, &a[1], &i__, &j, &descA[1], &work[1], &a[1], 
