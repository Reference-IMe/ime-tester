...
/*
 * @generated c Sun Nov 17 07:03:10 2013
 */
...
static void recover_checksum( char side, complexfloat_t *A, int m, int n, int *descA, ftla_work_t *ftw );
static void recover_abft( complexfloat_t *A, int m, int n, int *descA, ftla_work_t *ftw );
static void hor_checkpointing( complexfloat_t *A, int *descA, int IA, int JA, int iscopy, ftla_work_t *ftw );
static void hor_checkpointing_local (complexfloat_t *A, int *descA, int IA, int JA, int iscopy, ftla_work_t *ftw);
static void recover_local (char side, complexfloat_t *A, int MA, int NA, int *descA, ftla_work_t *ftw);
static void recover_local_fillin (complexfloat_t *A, int MA, int NA, int *descA, ftla_work_t *ftw);
static void create_matrix( int ctxt, complexfloat_t **A, int *descA, int M, int N, int NB, int *np_A, int *nq_A);
 ...
 ***** RECOVERY 
 */
void Cftla_pccsumr ( char zone, complexfloat_t *A, int m, int n, int *descA, ftla_work_t *ftw)
{
    ...
    cprintmatrix(A, descA, "Af_hole", 7);

    // recover checksum
    if( zone == 'C' || zone == 'A' ) {
        recover_checksum ('b', A, m, n, descA, ftw );
        ROOTSAY ("checksum recovered");
        cprintmatrix(A, descA, "Af_csum", 7);
    }
    ...
    if( zone == 'I' || zone == 'A' ) {
        ...
        cprintmatrix(A, descA, "Af_abft", 7);
    }
    ...
    if( zone == 'L' || zone == 'A' ) {
        ...
        cprintmatrix(A, descA, "Af_lcpy", 7);
    }
}
extern void ftla_pccsumr_( char *zone, complexfloat_t *A, int *m, int *n, int *descA, int *ftw ) {
    Cftla_pccsumr( zone[1], A, *m, *n, descA, (ftla_work_t*)ftw );
}

/*
 * Use the duplicate checksum to restore missing checksum blocks
 */
static void recover_checksum (char side, complexfloat_t *A, int MA, int NA, int *descA, ftla_work_t *ftw)
{
    int i, j, k;
    int tt[2], g[2];
    ...
    if (side == 'b' || side == 'r')
    {
        ...
        //Cpcgemr2d (MA-IA+1, nb, A, IA, tc, descA, A, IA, tc+nb, descA, ctxt);
        int i_c, j_c;
        if (iamvic)
        {
            i_c = np_A;
        ...
        for (i=0; i<i_c; i+=nb)
            for (j=0, k=0; j<j_c; j+=nb, k++)
            {
                ...
                Cpcgemr2d (nb, nb, A, tt[0], tt[1], descA, A, g[0], g[1], descA, ctxt);
            }
    }
}
 ...
 */
static void recover_abft (complexfloat_t *A, int MA, int NA, int *descA, ftla_work_t *ftw)
{
    int j;
    int lrindx, lcindx, rsrc, csrc;
    complexfloat_t *Achk_old, *Achk_new;
    ...
    Cpcgemr2d (MA, NAA-NA, A, 1, NA+1, descA, Achk_old, 1, 1, descAchk, ctxt);

    // re-calculate the checksum
    Cftla_pccsum( 'r', A, MA, NA, 1, 1, descA, A, 1, NA+1, descA, ftw );

    // copy the new checksum into buffer
    Cpcgemr2d (MA, NAA-NA, A, 1, NA+1, descA, Achk_new, 1, 1, descAchk, ctxt);

    int N_M = NAA-NA, i1=1;
    complexfloat_t done = 1, mone = -1;
    pcmatadd_ ( &MA, &N_M, &done, Achk_old, &i1, &i1, descAchk, &mone, Achk_new, &i1, &i1, descAchk);
    Cpcgemr2d (MA, NAA-NA, Achk_new, 1, 1, descAchk, A, 1, NA+1, descA, ctxt);
    ...
    for (j=0; j<j_c; j+=nb)
    {
        ...
        if (csrc==(ftw->errproc%npcol))
        {
            ...
            if (iamvic)
            {
                ...
                memcpy (A+j*np_A, A+(lcindx-1)*np_A, np_A*nb*sizeof(complexfloat_t));
            }
        }
        else
        {
            ...
            if (myrow == rsrc)
            {
                ...
                if (iamvic)
                {
                    ...
                    MPI_Recv(A+j*np_A, np_A*nb, MPI_COMPLEX, csrc, 1, ftw->comms.rowcomm, &status);
                }
                else if (mycol==csrc)
                {
                    //printf ("(%d,%d) sending to (%d,%d) for MPI_COPY when j=%d\n", myrow, mycol, myrow, ftw->errproc%npcol, j);
                    MPI_Send(A+(lcindx-1)*np_A, np_A*nb, MPI_COMPLEX, ftw->errproc%npcol, 1, ftw->comms.rowcomm);
                }
            }
        }
    }
    ...
    Cpcgemr2d (MA, NAA-NA, Achk_old, 1, 1, descAchk, A, 1, NA+1, descA, ctxt);
...
static void recover_local_fillin (complexfloat_t *A, int MA, int NA, int *descA, ftla_work_t *ftw)
{
    ...
    // copy the old checksum into buffer
    Cpcgemr2d (MA, nb*2, A, 1, NA+1, descA, Achk_old, 1, 1, descAchk, ctxt);

    // re-calculate the checksum
    hor_checkpointing_local (A, descA, 1, 1, 1, ftw );

    // copy the new checksum into buffer
    Cpcgemr2d (MA, nb*2, A, 1, NA+1, descA, Achk_new, 1, 1, descAchk, ctxt);

    int N_M = nb*2, i1=1;
    complexfloat_t done = 1, mone = -1;
    pcmatadd_ ( &MA, &N_M, &done, Achk_old, &i1, &i1, descAchk, &mone, Achk_new, &i1, &i1, descAchk);
    Cpcgemr2d (MA, nb*2, Achk_new, 1, 1, descAchk, A, 1, NA+1, descA, ctxt);
    ...
    // copy the correct checksum back //
    Cpcgemr2d (MA, nb*2, Achk_old, 1, 1, descAchk, A, 1, NA+1, descA, ctxt);
 ...
 */
void Cftla_pcqplcpy( complexfloat_t *A, int *descA, int IA, int JA, ftla_work_t *ftw ) {
    ...
    int len = (np_A-lrindx+1)*sizeof(complexfloat_t);
...
extern void ftla_pcqplcpy_(complexfloat_t *A, int *descA, int *IA, int *JA, int *ftw ) {
    Cftla_pcqplcpy( A, descA, *IA, *JA, (ftla_work_t*)ftw );
}

/**
 * Reset the local copy of the panel in the current Q-wide section
 */
void Cftla_pcqplrst( complexfloat_t *A, int *descA, int IA, int JA, ftla_work_t *ftw )
{
#ifndef NO_QLOCALCOPY
    int i;
    complexfloat_t *Aoff;
...
extern void ftla_pcqplrst_(complexfloat_t *A, int *descA, int *IA, int *JA, int *ftw ) {
    Cftla_pcqplrst( A, descA, *IA, *JA, (ftla_work_t*)ftw );
}
...
static void hor_checkpointing_local( complexfloat_t *A, int *descA, int IA, int JA, int iscopy, ftla_work_t *ftw )
{
    ...
    // temp buffers
    complexfloat_t *S = ftw->comms.S;
    complexfloat_t *R = ftw->comms.R;
...
//        printf ("(%d,%d) is root (csrc=%d) when JA=%d\n", myrow, mycol, csrc, JA);
    ...
    for (i=0; i<rows; i+=nb)
    {
        ...
        if (in)
        {
            ...
            for (k=0; k<nb; k++)
                memcpy (S+k*nb, Aoff+k*np_A, nb*sizeof(complexfloat_t));
        }
        else
            memset (S, 0, nb*nb*sizeof(complexfloat_t));

        if ((lrindx+i)<np_A)
        {
            MPI_Reduce (S, R, nb*nb, MPI_COMPLEX, MPI_SUM, csrc, ftw->comms.rowcomm);

            if (root)
            {
                Aoff = A+(rootcc-1)*np_A+(rootcr-1)+i;  
                for (k=0; k<nb; k++)
                    memcpy (Aoff+k*np_A, R+k*nb, nb*sizeof(complexfloat_t));
            }
        }
    }
 ...
 */
void Cftla_pccsum (char side,
        complexfloat_t *A, int MA, int NA, int IA, int JA, int *descA, 
        complexfloat_t *Acd, int IAcd, int JAcd, int *descAcd,
        ftla_work_t *ftw)
{
    ...
    if (side=='d' || side=='b') {
        ...
        if (nq_A > nqq_A)
            memset (A+np_A*nqq_A, 0, (nq_A-nqq_A)*np_A*sizeof(complexfloat_t));
...
extern void ftla_pccsum_( char *side, complexfloat_t *A, int *MA, int *NA, int *IA, int *JA, int *descA, complexfloat_t *Acd, int *IAcd, int *JAcd, int *descAcd, int *ftw ) {
    Cftla_pccsum( side[1], A, *MA, *NA, *IA, *JA, descA, Acd, *IAcd, *JAcd, descAcd, (ftla_work_t*)ftw );
}
...
static void hor_checkpointing (complexfloat_t *A, int *descA, int IA, int JA, int iscopy, ftla_work_t *ftw)
{
    ...
    if (iscopy && (lcindx<nq_A) && (lrindx+i)<np_A)
    {
        ...
        int len = (np_A-lrindx)*sizeof(complexfloat_t);
        Aoff = A+(lcindx-1)*np_A+(lrindx-1);
        for (i=0; i<nb; i++)
            memcpy (((typeof(A))ftw->pcopy.Pc)+i*np_A, Aoff+i*np_A, len);
    }
 ...
 */
float Cftla_pccsumv( int M, int N, complexfloat_t *A, int *descA, ftla_work_t *ftw) {
    complexfloat_t *Achk_old, *Achk_new;
    int descAchk[9], npp, nqq;
    int nb = descA[NB_];
    int ctxt = descA[CTXT_];
    ...
    // copy the old checksum into buffer
    Cpcgemr2d (M, N-M, A, 1, M+1, descA, Achk_old, 1, 1, descAchk, ctxt);

    // re-calculate the checksum
    Cftla_pccsum ('r', A, M, M, 1, 1, descA, A, 1, M, descA, ftw );

    // copy the new checksum into buffer
    Cpcgemr2d (M, N-M, A, 1, M+1, descA, Achk_new, 1, 1, descAchk, ctxt);

    // cross check
    int N_M = N-M, i1=1;
    complexfloat_t done = 1, mone = -1;
    pcmatadd_ ( &M, &N_M, &done, Achk_new, &i1, &i1, descAchk, &mone, Achk_old, &i1, &i1, descAchk);
    float resid = pclange_("F", &M, &N_M, Achk_old, &i1, &i1, descAchk, NULL)/pclange_("F", &M, &N_M, Achk_new, &i1, &i1, descAchk, NULL)/M;
...
extern float ftla_pccsumv_( int *M, int *N, complexfloat_t *A, int *descA, int *ftw ) {
    return Cftla_pccsumv( *M, *N, A, descA, (ftla_work_t*)ftw );
}


static void create_matrix( int ctxt, complexfloat_t **A, int *descA, 
