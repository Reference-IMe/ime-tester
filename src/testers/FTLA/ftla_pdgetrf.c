...
/* 
 * @generated d Sun Nov 17 07:17:03 2013
 */
...
/* Subroutine */ int ftla_pdgetrf(int *m, int *n, double *a, int *
        ia, int *ja, int *descA, int *ipiv, int *info, ftla_work_t *ftw)
{
    ...
    extern /* Subroutine */ int pdgemm_(char *, char *, int *, int *, 
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        pxerbla_(&ictxt, "FTPDGETRF", &i__1, (ftnlen)7);
        return 0;
    }

    /*     Quick return if possible */
    if (descA[3] == 1)  //num di colonne nella global matrix
    {
        ipiv[1] = 1;
        return 0;
    } 
    ...
    pb_topget_(&ictxt, "Broadcast", "Columnwise", colbtop, (ftnlen)9, (
                ftnlen)10, (ftnlen)1);
    pb_topget_(&ictxt, "Combine", "Columnwise", colctop, (ftnlen)7, (ftnlen)
            10, (ftnlen)1);
    pb_topset_(&ictxt, "Broadcast", "Rowwise", "S-ring", (ftnlen)9, (ftnlen)
            7, (ftnlen)6);
    pb_topset_(&ictxt, "Broadcast", "Columnwise", " ", (ftnlen)9, (ftnlen)10,
            (ftnlen)1);
    pb_topset_(&ictxt, "Combine", "Columnwise", " ", (ftnlen)7, (ftnlen)10, (
                ftnlen)1);
    ...
    if ( !Cftla_replay( ftw ) ) {
        Cftla_pdqplcpy (&a[1], &descA[1], *ia, *ja, ftw ); /* make a local copy & checkpoint localcopy */
    ...
    if (jb + 1 <= realn ) 
    {
        if ( !Cftla_replay( ftw ) ) {
            ...
            i__1 = *n - jb;
            i__2 = jn + 1;
            pdlaswp_("Forward", "Rows", &i__1, &a[1], ia, &i__2, &descA[1], ia, &in, 
        ...
        if (jb + 1 <= *m) 
        {
            ...
            i__5 = in + 1;
            i__6 = jn + 1;
            pdgemm_("No transpose", "No transpose", &i__1, &i__2, &jb, &c_b34,
        ...
        if ( Cftla_repairstep( ftw ) ) {
            //if(myrow+mycol==0) 
            fprintf(stderr, "### REPARING a failure at iteration %d(TRF) process %d\n", *ja, ftw->errproc);
            Cftla_pdcsumr ('A', &a[1], *m, realn, &descA[1], ftw );
...
/*
#ifdef INJECT
        dinject_errors(errors, j, &a[1], &descA[1], ftw );
...
//#endif
        ...
        if (j - *ja + jb + 1 <= realn) 
        {
            ...
            if (j - *ja + jb + 1 <= *m) 
            {
                ...
                i__7 = i__ + jb;
                i__8 = j + jb;
                pdgemm_("No transpose", "No transpose", &i__3, &i__4, &jb, &
       ...
       /*
        * now that pdgemm is done, see if we need to Recover 
        */

       /*
        * now that pdlarfb is done, see if we need to Recover 
        */
        //printf("ftw->errproc = %d\n",ftw->errproc);
        if ( Cftla_repairstep( ftw ) ) {
           //printf("Before: Cftla_repairstep( ftw )=%d Cftla_procfailed( ftw, j )=%d\n", Cftla_repairstep( ftw ), Cftla_procfailed( ftw, j ));
           if(mycol+myrow==0) fprintf(stderr, "### REPARING a failure at iteration %d(TRF) process %d (op %d>=%d)\n", j, ftw->errproc, ftw->curop, ftw->errop);
...
#ifdef VERIFY_CHK
    double resid = Cftla_pdcsumv (*m, *n, &a[1], &descA[1], ftw );
    ...
    pb_topset_(&ictxt, "Broadcast", "Columnwise", colbtop, (ftnlen)9, (
                ftnlen)10, (ftnlen)1);
    pb_topset_(&ictxt, "Combine", "Columnwise", colctop, (ftnlen)7, (ftnlen)
            10, (ftnlen)1);
 ...
 */
static int pdgetrf0( int *m, int *n, double *a, int *ia, int *ja, int *descA, 
                    int *ipiv, int *info, 
                    ftla_work_t *ftw );
static void qpanel_replay( double *A, int MA, int NA, int *descA, ftla_work_t *ftw )
{
    ...
    pdgetrf0 (&MM, &NN, A, &jj, &jj, descA, ipiv, &info, ftw );
    free (ipiv);
    ftw->errstep = 0;
    ftw->errproc = -1;
    ftw->errop = 0;
...
/* Subroutine */ int pdgetrf0(int *m, int *n, double *a, int *
        ia, int *ja, int *descA, int *ipiv, int *info, ftla_work_t *ftw)
{
    /* System generated locals */
    int i__1, i__2, i__3, i__4, i__5, i__6, i__7, i__8;
    ...
    *info = 0;
    ...
    {
        i__1 = -(*info);
        pxerbla_(&ictxt, "PDGETRF0", &i__1, (ftnlen)8);
        return 0;
    }
