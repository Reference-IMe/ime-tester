...
#include "../pviDGESV_WO.igather.h"


double test_IMe_pviDGESV_igather(const char* label, int verbosity, int rows, int cols, int nrhs, int rank, int cprocs, int sprocs)
{
	clock_t start, stop;
	double span, maxspan;
	double** A2;
	...
	if (i_calc)
	{
		...
		pviDGESV_WO_igather(rows, A2, nrhs, bb, xx, comm_calc);
