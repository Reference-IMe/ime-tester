 ...
 *
 */
void pviDGESV_WO_allgather(int n, double** A, int m, double** bb, double** xx, MPI_Comm comm)
{
    int rank, cprocs; //
	...
	for (l=n-1; l>0; l--)
	{
		...
		*/
		...
		MPI_Allgather (&Tlocal[l-1][local[n]], myTcols/2, MPI_DOUBLE, &TlastKr[0], 1, TlastKr_chunks_resized, comm);
		...
		MPI_Bcast (&TlastKc[0], Tcols/2, MPI_DOUBLE, map[l-1], comm);
	}
