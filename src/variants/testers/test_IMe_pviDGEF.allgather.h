...
#include "../pviDGEF_WO.allgather.h"


duration_t test_IMe_pviDGEF_allgather(const char* label, int verbosity, int rows, int cols, int rank, int cprocs, int sprocs)
{
	...
	if (i_calc)
	{
		...
		info = pviDGEF_WO_allgather(rows, A2, K, comm_calc);
